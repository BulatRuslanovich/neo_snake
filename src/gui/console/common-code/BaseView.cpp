...
void s21::BaseView::DrawBox(int topLeftY, int topLeftX, int bottomRightY, int bottomRightX) {
...
void s21::BaseView::GameFieldRendering(s21::Choose currentGame, int lvl, int score, int record) {
    ...
    DrawBox(0, 0, Properties::FIELD_HEIGHT + 1, Properties::FIELD_WIDTH + 1);

    mvaddch(0, Properties::FIELD_WIDTH + 1, ACS_TTEE);
    mvaddch(Properties::FIELD_HEIGHT + 1, Properties::FIELD_WIDTH + 1, ACS_BTEE);
    ...
    if (currentGame == Choose::TETRIS) {
        ...
        mvprintw(1, Properties::FIELD_WIDTH + 3, "Next");
        sidePanelHeight++;
    }

    DrawBox(sidePanelHeight, Properties::FIELD_WIDTH + 2, sidePanelHeight + 2,
            Properties::WINDOW_WIDTH - 1);
    mvprintw(sidePanelHeight, Properties::FIELD_WIDTH + 3, "Level");
    mvprintw(sidePanelHeight + 1, Properties::FIELD_WIDTH + 8, "%d", lvl);

    sidePanelHeight += 3;

    DrawBox(sidePanelHeight, Properties::FIELD_WIDTH + 2, sidePanelHeight + 2,
            Properties::WINDOW_WIDTH - 1);
    mvprintw(sidePanelHeight, Properties::FIELD_WIDTH + 3, "Score");
    mvprintw(sidePanelHeight + 1, Properties::FIELD_WIDTH + 9 - static_cast<int>(std::to_string(score).length()), "%d", score);

    sidePanelHeight += 3;

    DrawBox(sidePanelHeight, Properties::FIELD_WIDTH + 2, sidePanelHeight + 2, Properties::WINDOW_WIDTH - 1);
    mvprintw(sidePanelHeight, Properties::FIELD_WIDTH + 3, "Record");
    mvprintw(sidePanelHeight + 1, Properties::FIELD_WIDTH + 9 - static_cast<int>(std::to_string(record).length()), "%d", record);
}
...
void s21::BaseView::GameResultRendering(bool isWin, int lvl, int score) {
    ...
    if (isWin) {
        mvprintw(2, Properties::WINDOW_WIDTH / 2 - 4, "YOU WIN!");
    } else {
        mvprintw(2, Properties::WINDOW_WIDTH / 2 - 5, "GAME OVER!");
    }

    mvprintw(4, Properties::WINDOW_WIDTH / 2 - 7, "Your score: %d", score);
    mvprintw(6, Properties::WINDOW_WIDTH / 2 - 7, "Your level: %d", lvl);
    mvprintw(8, Properties::WINDOW_WIDTH / 2 - 6, "Press any key");
    mvprintw(9, Properties::WINDOW_WIDTH / 2 - 5, "to continue");
...
void s21::BaseView::PauseRendering(int level, int score) {
    ...
    mvprintw(5, Properties::WINDOW_WIDTH / 2 - 10, "The game is on pause");
    mvprintw(8, Properties::WINDOW_WIDTH / 2 - 7, "Your score:  %d", score);
    mvprintw(10, Properties::WINDOW_WIDTH / 2 - 7, "Your level:  %d", level);

    mvprintw(12, Properties::WINDOW_WIDTH / 2 - 10, "Press Tab to continue");
    mvprintw(13, Properties::WINDOW_WIDTH / 2 - 7, "or Esc to exit");
    nodelay(stdscr, FALSE);
}
void s21::BaseView::StartGameRendering() {
    clear();
    DrawBox(0, 0, Properties::WINDOW_HEIGHT, Properties::WINDOW_WIDTH);
    mvprintw(9, Properties::WINDOW_WIDTH / 2 - 6, "Press space");
    mvprintw(10, Properties::WINDOW_WIDTH / 2 - 7, "to start game");
    mvprintw(11, Properties::WINDOW_WIDTH / 2 - 7, "Or Esc to exit");
    nodelay(stdscr, FALSE);
}
...
void s21::BaseView::MenuRendering(size_t &selectedItem) {
    ...
    int startY = 10;
    int startX = Properties::WINDOW_WIDTH / 2;
